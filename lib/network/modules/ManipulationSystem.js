...
class ManipulationSystem {
  ...
  /**
   * create the edges from node itself [OpManager]
   */

  addEdgeModeCustom(event) {
    // when using the gui, enable edit mode if it wasnt already.
    if (this.editMode !== true) {
      this.enableEditMode();
    }
    ...
    this._temporaryBindUI('onDrag',    this._dragControlNode.bind(this));
    this._temporaryBindUI('onRelease', this._finishConnectCustom.bind(this));
  ...
  /**
   * Connect the new edge to the near by node
   * @param event
   * @private
   */
  _finishConnectCustom(event) {
    let pointer = this.body.functions.getPointer(event.center);
    let pointerObj = this.selectionHandler._pointerToPositionObject(pointer);
